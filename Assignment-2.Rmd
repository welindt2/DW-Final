...
noregMetrics<-c(noregLL, noregAUC, noregACC, noregTPR, noregTNR, noregPRE)
...
ridgeMetrics<-c(ridgeLL, ridgeAUC, ridgeACC, ridgeTPR, ridgeTNR, ridgePRE)
...
#Predict test set
```{r}
final.gbm <- my.models[[10]]

# Predict the probabilities for the observations in the test dataset

predicted_test <- predict(final.gbm, testSet, type='prob')
...
```{r}
gradientTPR<- confusion[2,2]/(confusion[2,1]+confusion[2,2])
gradientTNR<- confusion[1,1]/(confusion[1,1]+confusion[1,2])
gradientPRE<-confusion[2,2]/(confusion[1,2]+confusion[2,2])
gradientACC<- (confusion[1,1]+confusion[2,2])/(confusion[1,1]+confusion[1,2]+confusion[2,1]+confusion[2,2])

gradientTPR
gradientTNR
gradientPRE
gradientLL<-ridge$results$logLoss


gradientAUC
gradientMetrics<-c(gradientLL, gradientAUC, gradientACC, gradientTPR, gradientTNR, gradientPRE)
